#!/bin/bash
#SBATCH --job-name=llava-debug-loss
...
OUTPUT_DIR="$SCRATCH/checkpoints/llava-debug-loss"
...
# ---- Single-GPU debug run: 10 steps, no checkpoint, no wandb ----
deepspeed --num_gpus=1 --master_port $MASTER_PORT llava/train/train_mem.py \
    --deepspeed ./scripts/zero3_offload_debug.json \
